// Manual regression check for docs/plano/ (the "UCL a Plano" module):
// serve docs/ locally, then:
...
const BASE_URL = process.argv[4] ?? "http://localhost:8899/plano/index.html";

if (DOCX_PATHS.length === 0 || !OUT_DIR) {
  console.error("Usage: node scripts/e2e-plano-test.mjs <path-to-competencia.docx>[,<path2.docx>,...] <output-dir> [base-url]");
  process.exit(1);
}
